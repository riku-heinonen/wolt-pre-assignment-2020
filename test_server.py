...
def coordinates():
    ...
    longitudes = []
    for restaurant in restaurants:
        longitudes.append(restaurant['location'][0])
        latitudes.append(restaurant['location'][1])
    return {
...
def test_random_queries(tags, coordinates):
    '''
        ...
        If the query matches any restaurants, check that the location is less than 3000 metres away from the queried location and the tags are included in the restaurants name, description or tags.
    '''
    for i in range(1000):
        lon = random.uniform(
            coordinates['min_lon'], coordinates['max_lon'])
        lat = random.uniform(
            coordinates['min_lat'], coordinates['max_lat'])
        location = (lon, lat)
        ...
        if response.data:
            ...
            for restaurant in response_list:
                assert any(tag in values for values in [restaurant['name'],
                                                        restaurant['description'],
                                                        restaurant['tags']])
                assert geopy.distance.distance(
                    location, restaurant['location']).m < 3000

...
def test_random_queries(tags, coordinates):
    """
        Make 1000 queries with random parameters chosen from all of the possible
        tags and random locations inside the bounding rectangle of the
        restaurants in the dataset. If the query matches any restaurants, check
        that the location is less than 3000 metres away from the queried
        location and the tags are included in the restaurant's name, description
        or tags.
    """
    for i in range(1000):
        lon = random.uniform(coordinates["min_lon"], coordinates["max_lon"])
        lat = random.uniform(coordinates["min_lat"], coordinates["max_lat"])
